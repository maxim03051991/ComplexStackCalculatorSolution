...
using System;
using System.Globalization; // Подключаем CultureInfo для парсинга чисел с точкой и инвариантной локалями
...
namespace ComplexNumbersLib // Объявляем пространство имён библиотеки
{
    ...
    public readonly struct Complex : IEquatable<Complex> // Объявляем неизменяемую структуру и интерфейс равенства
    {
        ...
        // Parse: преобразует строку в комплексное число. 
        ...
        // Разделитель дробной части: точка (InvariantCulture).
        ...
        //TryParse: безопасный разбор строки, возвращает true/false.
        public static bool TryParse(string? s, out Complex value) // Метод безопасного парсинга
        {
            ...
            // 1) Попытка распарсить как пару (Re,Im) в форматах "(a,b)" "(a;b)" "a,b" "a;b" 
            if (TryParsePair(t, out value)) // Если получилось распарсить как пару
                return true; // Успех

            // 2) Если нет 'i' — это чисто вещественное число
            ...
            // 4) Общий случай: должны оканчиваться на 'i' (например "3+4i" или "4i") 
            if (!t.EndsWith("i")) // Если 'i' не в конце — формат неверный
                return false; // Сообщаем об ошибке

            string withoutI = t.Substring(0, t.Length - 1); // Убираем завершающий символ 'i'

            // 5) Ищем разделитель между Re и Im: '+' или '-' не в начале (например "3+4" или "3-4") 
            ...
            // 6) Если разделитель есть — слева Re, справа Im (со знаком) 
        ...
        private static bool TryParsePair(string t, out Complex value) // Парсинг пары (Re,Im)
        {
            ...
            // Поддерживаем разделители ',' и ';' между Re и Im 
        ...
        {
            // Быстрый способ убрать пробелы без LINQ 
    ...
    // Класс стандартных математических констант
